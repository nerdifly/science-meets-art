...
base_frames = 200
slow_factor = 3    # >1 = slower evolution
...
# ANIMATION FUNCTION
def animate_loop_color_to_gif(filename):
    ...
    # Dynamic text placeholders for s and g
    ...
    def update(frame):
        ...
        lc.set_array((theta + frame*color_speed) % (2*np.pi))  # color animation
    ...
    writer = PillowWriter(fps=30)  # GIF writer
    anim.save(filename, writer=writer)
    plt.close(fig)

# RENDER LOOPABLE COLOR ANIMATION TO GIF
animate_loop_color_to_gif("original_animation_with_s_g.gif")